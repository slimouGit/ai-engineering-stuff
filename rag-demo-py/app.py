...
def ask():
    try:
        ...
        if not document_name:
            raise ValueError("Bitte ein Dokument auswählen.")

        if not question:
            raise ValueError("Bitte eine Frage eingeben.")

        result = rag_service.ask(document_name, question)
...
def render_page(
    ...
    try:
        ...
        embedding_models = [m for m in all_models if any(t in m.lower() for t in embedding_terms)]
        chat_models = [m for m in all_models if not any(t in m.lower() for t in embedding_terms)]
    except Exception as e:
        model_error = str(e)
